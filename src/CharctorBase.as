package
{
	...
	public class CharctorBase extends ActorParam implements Iactor
	{
		...
		////////////////////////////////////
		public function CharctorBase(srcCharacterAtlasBitmap:Bitmap ,
									 colorTransform:ColorTransform ,
									 _motionToAssetFrameIds:Array ,
									 _motionToWeight:Array ,
									 _motionToInputAllowance:Array ,
									 $$D:Array ,
									 _motionContinue:Array ,
									 _motionToHitDetection:Array)
		{

			super(srcCharacterAtlasBitmap , colorTransform , 256);
		 ...
		 *
		 */
		public function input(keyA:Boolean , //攻撃ボタン   攻击按钮
		...
		):void
		{
			...
			if (_input_jump == 1 && _command2 == 0)
			{
				_command2 = 2;
				_command2CNT = 0;
			}
			if (_inputRight == 1 && _command2 == 2 && _command2CNT < 10)
			{
				_command2 = 3;
				_command2CNT = 0;
			}
			...
			{//アニメーション
				...
				if (motionToWeight[_action][_actionstep] <= _animwait)
				{
					...
					switch (motionContinue[_action][_actionstep])
					{
						...
						case 4:
							_velocity.y = -2;
							_velocity.x = _stepPower;
							break;
						case 5:
							_dirc = (_inputLeft) ? true : ((_inputRight) ? false : _dirc);
							break;
						default:
							break;
					}
...
//				if (10 * 10 < L)
		...
		public function render():void
		{
			...
			var yo:int = scaledSize * 3 / 4
			_renderpos.x = _pos.x - scaledSize / 2;
			_renderpos.y = _pos.z + _pos.y - yo;
			...
			_r.x = _renderpos.x + scaledSize / 4;
			_r.y = _renderpos.y + yo;
			_r.width = scaledSize / 2;
			_r.height = 5;
			Global._canvas.fillRect(_r , 0x000000);

			_r.x = _renderpos.x + 1 + scaledSize / 4;
			_r.y = _renderpos.y + yo + 1;
			_r.width = scaledSize / 2 - 2;
			_r.height = 5 - 2;
			Global._canvas.fillRect(_r , 0xFF0000);

			_r.x = _renderpos.x + 1 + scaledSize / 4;
			_r.y = _renderpos.y + yo + 1;
			_r.width = (scaledSize / 2 * _hp / DEFAULT_HP) - 2;
			_r.height = 5 - 2;
